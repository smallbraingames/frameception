...
const createOrFindEmbeddedWalletForFid = async (
  fid: number,
  ownerAddress: string
  ...
  if (existingAddress) return getAddress(existingAddress);
  ...
  return newAddress ? getAddress(newAddress) : undefined;
};
...
export default createOrFindEmbeddedWalletForFid;
